public class EstMagiqueNormal{
	...
	public static boolean estMagiqueNormal(int[][] m){
		...
		verif = new boolean[m.length * m.length + 1];
		i = 0;
		while(i < m.length){
			j = 0;
			while(j <m[i].length){
				if(m[i][j] < 1 || m[i][j] > (m.length * m.length)){
					return false;
				}
				if (verif[m[i][j]] == true ){
					return false;
		...
		System.out.println("Est un carrée magique normal");
		return estMagique(m);
	}
